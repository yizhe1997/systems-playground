...
| `dsh-send-link.yml` | Manual only (`workflow_dispatch`) | Reads dsh's launch link on the host and posts it to a private Discord webhook (`DSH_LINK_WEBHOOK_URL` in Infisical), masked in the run log, so a new browser can be enrolled without shell access |
