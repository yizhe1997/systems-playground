...
`docker-compose.override.yml` is auto-loaded for local dev (builds from source). Production/self-hosted runs instead merge `docker-compose.prod.yml` (pre-built Docker Hub images + Watchtower labels) — see [`docs/DEPLOYMENT_WSL.md`](DEPLOYMENT_WSL.md).
...
2. **Update agent guidance files** — `self-host/apps/portfolio/frontend/AGENTS.md` and `self-host/apps/portfolio/frontend/CLAUDE.md` already demonstrate the pattern. Create equivalent files for the new agent and point them at the same content.
...
4. Register your project in `self-host/apps/portfolio/frontend/src/app/projects/page.tsx` so it appears in the showcase.
