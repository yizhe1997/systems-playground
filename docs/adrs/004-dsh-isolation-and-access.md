...
- **Origin does not verify the Access token.** Cloudflare recommends that the origin, or `cloudflared`, validates the Access JWT so a request that bypasses Access is rejected. `cloudflared` can do this per ingress rule (`originRequest.access`), but `cloudflared-sync.sh` only emits `hostname`/`service` lines, so it is not enabled. If the Access application were deleted or mis-scoped, dsh's own launch-token/cookie is the only remaining gate. A follow-up could teach the sync script to emit that block for labelled containers.
