...
- **A build-time patch of dsh's browser client.** dsh's client enables host settings only when the page hostname is loopback, so from `dsh.<HOST>` Settings → Models fails with "settings are unavailable in this browser". `self-host/infra/dsh/patch-remote-settings.js` rewrites the two client-side checks (`ctx.remote.$host.isLoopback ?` in the settings and General-settings clients) so host settings are always used. Verified on 2026-09-20 against a non-loopback hostname: the unpatched image reproduced the error, the patched one loaded Settings → Models and a settings change was written to `settings.yaml`. Rationale: the check is client-side only (the settings service has no loopback check), every request already has to pass Access, the trusted-host fence and the session cookie, and the agent runs as the same user with a writable `dsh_home` volume, so it could edit `settings.yaml` regardless. Cost: the patch touches third-party code and must be re-reviewed on every dsh upgrade; it fails the image build unless each expected line matches exactly once, so drift is loud. If it ever stops working, the loopback launch link (`link.sh local`, also sent by the launch-link workflow) still opens settings from a browser on the host.
