...
### Context
Every deploy workflow today (`deploy-app-portfolio.yml`, `deploy-infra-n8n.yml`, `deploy-infra-uptime-kuma.yml`, `deploy-infra-watchtower.yml`) follows the same pattern: GitHub Actions secrets/vars are read into the job's environment, then written out with a shell heredoc — `cat <<EOF > "$TARGET/.env" ... EOF` — onto the self-hosted runner's disk, where `docker compose` reads them via `env_file: .env`.
...
**Boot-time implication:** because the values are baked into the container at creation time (during a CI-triggered deploy, via `infisical run`), a plain host reboot does **not** need to reach Infisical at all — `wsl-startup.sh` just restarts already-existing containers with their already-baked env, same as today. Infisical only needs to be reachable at the moment a container is *created or recreated* (i.e., during a CI deploy), not on every boot — with one exception: a disaster-recovery scenario (fresh host, no pre-existing containers) requires Infisical to be up and already bootstrapped before any other service's first-ever deploy can succeed. `wsl-startup.sh` now starts `infisical` first and waits for its `/api/status` health check before starting anything else, both to cover that fresh-host case and defensively, in case a future service's own container entrypoint calls Infisical directly at startup rather than only at CI deploy time.
...
- Infisical's own bootstrap credentials (DB password, encryption key, auth secret — see `self-host/infra/infisical/.env.example`) cannot themselves live in Infisical; this is an unavoidable root-of-trust problem shared by every secrets manager, and those specific values remain a plaintext `.env` on the runner, same as today. Everything *downstream* of Infisical being up is what moves out of plaintext.
