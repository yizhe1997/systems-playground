...
- **No workflow that targets the self-hosted runner (`runs-on: self-hosted`) may trigger on `pull_request`.** If PR-triggered checks are wanted (e.g. build/lint on PRs), they must run on GitHub-hosted runners (`runs-on: ubuntu-latest`), which build-app-*.yml already does for the Docker Hub build/push steps.
