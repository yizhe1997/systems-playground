...
**Date:** 2026-03-27
**Status:** Accepted — context updated 2026-07-30. The originating scenario (container start/stop via `docker.sock`) was removed by [ADR 004](./004-retire-live-playground.md); the pattern itself is unaffected and now protects the CMS, resume-request, and config admin routes instead.

### Context
Originally written when the Golang Control Plane API could physically start and stop Docker containers on the host machine via `/var/run/docker.sock` (see [ADR 001](./001-custom-go-control-plane.md)). That capability, and the socket mount it depended on, was removed entirely by [ADR 004](./004-retire-live-playground.md) — the admin surface today is the CMS manager, resume-request approve/reject/retriage, and site config, not container orchestration.

The underlying problem this ADR solves is unchanged: the Next.js frontend needs to allow authenticated "Admin" users to call privileged Go backend endpoints (`/admin/*`) while blocking "Viewer" users, without exposing the shared secret to the browser.
...
### Reasoning
1. **Zero-Trust Client Security:** Option 1 is inherently insecure for Single Page Applications (SPAs). Any variable exposed to the browser (`NEXT_PUBLIC_`) can be extracted via Chrome DevTools. If a "Viewer" extracted the PSK, they could bypass the UI role restrictions and `curl` the Go backend directly to hit privileged admin endpoints (CMS edits, resume-request approval, sending the applicant's resume, config changes).
