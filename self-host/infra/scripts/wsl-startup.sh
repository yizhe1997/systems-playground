...
# (self-host/infra/scripts/systemd/systems-playground-infra.service, installed by
# scripts/bootstrap.sh) ordered `After=docker.service`/`Requires=docker.service`, so systemd
# already guarantees dockerd is up before this script's first line runs.
...
# Cloudflare tunnel + cloudflared-sync are no longer started here - they run as their own
# `Restart=always` systemd user units (self-host/infra/scripts/systemd/cloudflared.service and
# cloudflared-sync.service, installed by scripts/bootstrap.sh), independent of this script's own
# lifecycle. That gets them real crash recovery (a dead tunnel process comes back on its own
# instead of staying down until the next full host reboot, which is what actually happened once)
# instead of the one-shot `nohup ... &` this script used to do.
