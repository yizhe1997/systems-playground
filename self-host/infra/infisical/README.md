...
| `DSH_LINK_WEBHOOK_URL` | `dsh-send-link.yml` | Only to use that workflow | — |
...
- dsh does too, and the `HOST` GitHub repo *variable* (your domain) is what publishes it through the tunnel. Hostnames are deliberately plain GitHub variables, not Infisical secrets: they are not secret (see [`../dsh/README.md`](../dsh/README.md)).
...
| `LOGDIR`, `TUNNEL_NAME`, `DISCORD_WEBHOOK_INFRA_ALERTS`, `BACKUP_DIR`, `BACKUP_RETENTION_DAYS`, `CLOUDFLARED_SYNC_ENABLED` ([`deploy-infra-scripts.yml`](../../../.github/workflows/deploy-infra-scripts.yml)) | Consumed by `wsl-startup.sh`/`wsl-shutdown.sh`/`wsl-backup.sh`/`cloudflared-sync.sh` at host boot via systemd (see [the scripts README](../scripts/README.md) and `docs/DEPLOYMENT.md` section 2) — no CI parent process for `infisical run` to inject into, and Infisical itself isn't running yet at that point anyway. Same chicken-and-egg problem as the row above. See the scripts README for which of these are actually required vs. optional. |
