...
- **Shared / cross-service:** `DOCKER_USERNAME` and `DOCKER_PASSWORD` — `DOCKER_USERNAME` isn't a secret in the traditional sense (it's just a Docker Hub username, and ends up public anyway as part of every pushed image tag), but every deploy workflow that pulls a self-built image (`docker-compose.prod.yml` for the portfolio's backend/frontend, `self-host/infra/n8n/docker-compose.yml`) interpolates `${DOCKER_USERNAME:-yourdockerhub}` to resolve which account to pull from — missing it silently falls back to the literal placeholder `yourdockerhub`, which doesn't exist, so the pull fails. `DOCKER_PASSWORD` is a real secret and is now required too: as of 2026-07-13, `build-app-portfolio-backend.yml`, `build-app-portfolio-frontend.yml`, and `build-infra-n8n.yml` all moved from GitHub Secrets + `ubuntu-latest` to `runs-on: self-hosted` fetching both values via `infisical run` (same pattern as everything else), after the GitHub Secrets were removed. Both must be in Infisical's `prod` env or every build fails at the `docker login` step.
