...
- **Shared / cross-service:** `DOCKER_USERNAME` — not a secret in the traditional sense (it's just a Docker Hub username), but every deploy workflow that pulls a self-built image (`docker-compose.prod.yml` for the portfolio's backend/frontend, and now `self-host/infra/n8n/docker-compose.yml`) interpolates `${DOCKER_USERNAME:-yourdockerhub}` to resolve which account to pull from. If it's missing from Infisical's `prod` env, every one of those pulls silently falls back to the literal placeholder `yourdockerhub`, which doesn't exist — the pull fails. This was already relied on for the portfolio app before this doc caught up to it; make sure it's actually set.
